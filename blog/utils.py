...
def filter_posts(request, query, posts):
    if query == 'my_posts':
        posts = posts.filter(author=request.user) if request.user.is_authenticated else posts
    ...
    if query == 'comment':
        posts = posts.annotate(num_comment=Count('comment')).order_by('-num_comment')
    if query == 'views':
        posts = posts.annotate(num_views = Count('views')).order_by('-num_views')
    if query == 'rate':
        posts = posts.annotate(rate = Count('likes') + Count('comment') + Count('views')).order_by('-rate')
...
def filter_comments(query, comments, request):
    if query == 'comment_popular':
        return comments.order_by('-comment_likes')
    if query == 'comment_old':
        return comments.order_by('date')
    if query == 'comment_new':
        return comments.order_by('-date')
    if query == 'comment_main':
        return comments.filter(user=request.user) if request.user.is_authenticated else comments