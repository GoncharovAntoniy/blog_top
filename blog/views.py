...
from .utils import filter_posts, do_action, add_saved, filter_comments
...
def home(request):
    ...
    posts = posts.filter(date__range=[from_date, to_date]) if from_date else posts
    ...
    post_pages = posts.paginator.get_elided_page_range(posts.number, on_each_side=1, on_ends=1)
...
def post(request, slug):
    ...
    comments = filter_comments(query, comments, request) if query else comments
...
def comment_like(request, pk):
    ...
    return redirect(f'blog:post', slug=comment.post.slug)
